...
import { Upload, FileText, CheckCircle, AlertCircle, Trash2, Files, RefreshCw, Barcode, Clock, FileSpreadsheet } from 'lucide-react';
import { api } from '../api';
...
function UploadPage() {
    ...
    const handleUpload = async () => {
        ...
        let totalPages = 0;
        let totalBarcodes = 0;
        let totalSerialsFromSheet = 0;
        const uploadResults = [];

        try {
            for (const file of files) {
                try {
                    const result = await api.uploadFile(file, serialFile);
                    ...
                    if (result.success) {
                        ...
                        totalSerialsFromSheet += result.stats?.serials_from_sheet || 0;
                        ...
                        uploadResults.push({
                            ...
                            serialsFromSheet: result.stats?.serials_from_sheet || 0,
            ...
            if (uploadedCount > 0) {
                ...
                setStats({
                    ...
                    serialsFromSheet: totalSerialsFromSheet
                });
                setFiles([]);
                ...
                <div className="card">
                    ...
                    <div
                        style={{
                            ...
                            marginTop: '18px',
                            cursor: 'pointer',
                            background: serialFile ? 'rgba(99,91,255,0.04)' : 'var(--bg-surface)'
                        }}
                        ...
                        <div className="flex items-center" style={{ gap: '12px' }}>
                            ...
                            <div style={{ minWidth: 0 }}>
                                <div style={{ fontWeight: 600, color: 'var(--text-main)', marginBottom: '4px' }}>
                                    {serialFile ? serialFile.name : 'Optional serial spreadsheet'}
                                </div>
                                <div className="text-muted" style={{ fontSize: '13px' }}>
                                    Use this for files like Serial Number.xlsx. PDF text matching is still checked first.
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div style={{ marginTop: '32px', textAlign: 'center' }} className="animate-in">
                            ...
                            {stats && (
                                <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                                    ...
                                    <div style={{ background: 'var(--bg-body)', padding: '20px', borderRadius: '8px' }}>
                                        <div style={{ fontSize: '24px', fontWeight: '700', color: 'var(--primary)' }}>{stats.serialsFromSheet || 0}</div>
                                        <div className="text-muted" style={{ fontSize: '13px', fontWeight: '500', textTransform: 'uppercase' }}>Spreadsheet Serials</div>
                                    </div>
                                    <div style={{ background: 'var(--bg-body)', padding: '20px', borderRadius: '8px' }}>
                                    ...
                                    {results.map((result, idx) => (
                                        <div key={`${result.name}-${idx}`} style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', padding: '6px 0', borderBottom: idx < results.length - 1 ? '1px solid var(--divider)' : 'none' }}>
                                            ...
                                            <span style={{ fontSize: '12px', color: result.success ? 'var(--success)' : 'var(--error)' }}>
                                                {result.success ? `${result.duplicate ? 'Already Exists' : 'Uploaded'} • ${result.barcodes} matches` : 'Failed'}
                                            </span>
                                        </div>
                                    ))}
                                </div>
