...
def health_check():
    return jsonify({'status': 'ok', 'message': 'PDF Print Cyient server is running'})
...
def upload_file():
    ...
    if file and file.filename.lower().endswith('.pdf'):
        ...
        serial_file = request.files.get('serials')
        serial_filepath = None
        if serial_file and serial_file.filename:
            serial_filename = secure_filename(serial_file.filename)
            if not serial_filename.lower().endswith(('.xlsx', '.xlsm', '.xltx', '.xltm')):
                return jsonify({'error': 'Serial file must be an Excel .xlsx file'}), 400
            serial_filepath = os.path.join(app.config['UPLOAD_FOLDER'], f"{uuid.uuid4().hex}_{serial_filename}")
            serial_file.save(serial_filepath)
        
        # Process PDF
        try:
            result = pdf_service.process_pdf(filepath, filename, serial_filepath)
