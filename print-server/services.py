...
class PDFProcessingService:
    ...
    def process_pdf(self, file_path, original_filename, serial_file_path=None):
        # Calculate Hash
        file_hash = self.calculate_file_hash(file_path)
        
        # Check for duplicates. If a serial spreadsheet is provided, refresh the
        # mappings for the existing PDF instead of silently skipping indexing.
        if file_hash in self.hashes:
            existing_id = self.hashes[file_hash]
            if not serial_file_path:
                ...
                return {
                    ...
                    'stats': {
                        ...
                        'serials_from_sheet': self.documents[existing_id].get('serials_from_sheet', 0)
                    },
                    'is_duplicate': True
                }

            logger.info(f"Duplicate file uploaded with spreadsheet. Re-indexing existing ID: {existing_id}")
            existing_doc = self.documents[existing_id]
            file_id = existing_id
            file_path = existing_doc['path']
            original_filename = existing_doc['name']
        ...
        doc_info = {
            ...
            'path': file_path,
            'uploaded_at': self.documents.get(file_id, {}).get('uploaded_at', datetime.datetime.now().isoformat()),
            'pages': 0,
            'barcodes_found': 0,
            'hash': file_hash,
            'serials_from_sheet': 0
        }
        ...
        for i, page in enumerate(reader.pages):
            ...
            serials = text_service.extract_serial_numbers(combined_text)
        ...
        if serial_file_path:
            serials_from_sheet, page_mappings = self._read_serial_spreadsheet(serial_file_path, doc_info['pages'])
            doc_info['serials_from_sheet'] = len(serials_from_sheet)

            for serial in serials_from_sheet:
                normalized_serial = self._normalize_barcode(serial)
                matched_page = page_mappings.get(normalized_serial)

                if matched_page is None:
                    matched_page = self._find_serial_in_page_texts(normalized_serial, page_texts)
        ...
        return {
            ...
            'stats': {
                ...
                'serials_from_sheet': doc_info['serials_from_sheet']
            },
            'is_duplicate': is_duplicate
        }

    def _add_mapping(self, barcode, file_id, page_num, label_type, confidence, doc_name, doc_info=None):
        barcode = str(barcode or '').strip()
        if not barcode:
            return False

        existing = self.mappings.get(barcode)
        if existing and existing.get('file_id') == file_id and existing.get('page_num') == page_num:
            return False
    ...
    def _read_serial_spreadsheet(self, spreadsheet_path, page_count):
        ...
        has_header = any(
            any(token in heading for token in ('serial', 'barcode', 'page'))
        ...
        if has_header:
            data_rows = rows[1:]
            serial_col = self._find_header_column(header, ('serial', 'barcode', 'code'), 0)
            page_col = self._find_header_column(header, ('page', 'sheet'), None)

        use_order_mapping = page_col is not None or len(data_rows) <= page_count
        serials = []
        page_mappings = {}
        ...
        for offset, row in enumerate(data_rows):
            ...
            serial = str(row[serial_col] or '').strip()
            normalized = self._normalize_barcode(serial)
            if not normalized:
                continue

            serials.append(serial)

            page_num = offset + 1 if use_order_mapping else None
            if page_col is not None and page_col < len(row) and row[page_col] not in (None, ''):
                try:
                    page_num = int(float(row[page_col]))
                except (TypeError, ValueError):
                    logger.warning("Invalid page number for serial %s", serial)
                    page_num = None

            if page_num is not None and 1 <= page_num <= page_count:
                page_mappings[normalized] = page_num

        return serials, page_mappings

    def _find_header_column(self, header, tokens, default):
        for index, heading in enumerate(header):
            if any(token in heading for token in tokens):
                return index
        return default

    def _find_serial_in_page_texts(self, normalized_serial, page_texts):
        if not normalized_serial:
            return None

        for index, text in enumerate(page_texts):
            if normalized_serial in self._normalize_barcode(text):
                return index + 1
        return None
...
class TextExtractionService:
    ...
    def extract_serial_numbers(self, text):
        ...
        patterns = [
            (r'\b([A-Z]{2,}\d{3,}-\d{3,})\b', 'HYPHENATED_SERIAL'),
            (r'\b(\d{3,}-\d{3,})\b', 'HYPHENATED_SERIAL'),
